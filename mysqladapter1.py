...
def get_meta_data(curw_id):
    sql = [
        ...
        "SELECT name, latitude, longitude, description FROM curw_iot.station WHERE id in (SELECT station FROM curw_iot.run WHERE id='" + str(curw_id) + "')",
        "SELECT variable FROM curw_iot.variable WHERE id in (SELECT variable FROM curw_iot.run WHERE id='" + str(curw_id) + "')",
        "SELECT unit, type FROM curw_iot.unit WHERE id in (SELECT unit FROM curw_iot.run WHERE id='" + str(curw_id) + "')"
    ]
...
def insert_id_rows(curw_id, obs_hash_id) :
  ...
  cursor.execute("""INSERT INTO curw_iot.id_mapping(curw_id, curw_obs_id) VALUES (%s,%s)""", (str(curw_id), str(obs_hash_id)))
  mydb.commit()
...
#def extract_timeseries(result):
    ...
    for i in values:
        ...
        sql = "SELECT time, value FROM curw_iot.processed_data WHERE id='%s' and time<='%s' and time>'%s'" % (str(event_id), end, start)
